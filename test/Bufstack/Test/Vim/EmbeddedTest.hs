...
{- |
- Adapted from Neovim.Test
-}
module Bufstack.Test.Vim.EmbeddedTest (
...
testWithEmbeddedNeovim
    ...
    -> ((Handle, Handle, ProcessHandle) -> Neovim env a)   -- ^ Test case
    -> IO ()
testWithEmbeddedNeovim file timeout r fTest =
    runTest `catch` catchIfNvimIsNotOnPath
  where
    runTest = do
        (hin, hout, ph, cfg, cleanUp) <- startEmbeddedNvim file timeout

        let testCfg = Internal.retypeConfig r cfg
            (Internal.Neovim a) = fTest (hin, hout, ph)
